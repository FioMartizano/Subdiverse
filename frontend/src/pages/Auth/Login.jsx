...
function Login() {
  ...
  const checkAccountStatus = async (userId) => {
    ...
    if (!userDocSnap.exists()) {
      return { exists: false, accountStatus: null, role: null };
    }
    ...
    return {
      ...
      // New field name. Keep data.status temporarily for older Firestore records.
      accountStatus: String(
        data.accountStatus || data.status || "pending"
      ).toLowerCase(),
  ...
  // Validate the Firestore profile and return the user's role.
  // Navigation happens only after the Firebase Auth session is confirmed.
  const routeUserByProfile = async (userId) => {
    const { exists, accountStatus, role } = await checkAccountStatus(userId);
    ...
    if (accountStatus === "archived") {
      return { success: false, reason: "archived" };
    }
    ...
    if (role !== "resident") {
      return { success: false, reason: "not_resident" };
    }
  ...
  /*
   * Confirm that the main Firebase Auth instance still owns the
   * authenticated user before entering protected resident/admin pages.
   */
  const ensureAuthSession = async (firebaseUser) => {
    if (!firebaseUser) {
      return false;
    }
    ...
    if (
      ...
      console.warn(
        "Auth session disappeared. Restoring current user..."
      );
    ...
    return auth.currentUser?.uid === firebaseUser.uid;
  };

  /*
   * Returns the correct message for blocked account statuses.
   * Both login methods use this so their behavior stays consistent.
   */
  const getAccountAccessMessage = (reason, isGoogleLogin = false) => {
    switch (reason) {
      case "pending":
        return "Your account is pending approval. Please wait for HOA approval before logging in.";

      case "rejected":
        return "Your registration was not approved. Please contact the HOA office.";

      case "frozen":
        return "Your account is temporarily frozen. Please contact the HOA office for assistance.";

      case "archived":
        return "This account has been archived because you are no longer listed as an active Windward resident. Please contact the HOA office if you have returned.";

      case "suspended":
        return "Your account has been suspended. Please contact the HOA office for more information.";

      case "inactive":
        return "Your account is currently inactive. Please contact the HOA office.";

      case "no_profile":
        return isGoogleLogin
          ? "This Google account has not completed registration yet. Go to Sign Up and choose Sign Up with Google."
          : "User profile not found in our records. Please contact the HOA office.";

      case "not_resident":
        return "Administrator accounts cannot log in through the Resident Portal. Please use the Admin Portal.";

      default:
        return "Unable to access this account because its role is invalid. Please contact the HOA office.";
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("AFTER EMAIL/PASSWORD LOGIN:", {
        currentUser: auth.currentUser,
        uid: auth.currentUser?.uid,
        email: auth.currentUser?.email,
      });
      ...
      /*
       * Password-based accounts must verify email ownership first.
       */
      ...
      const {
        ...
        reason,
        role
      } = await routeUserByProfile(
        signedInUser.uid
      );

      console.log("EMAIL/PASSWORD PROFILE CHECK:", {
        success,
        reason,
        role,
        currentUser: auth.currentUser,
      ...
      if (!success) {
        ...
        setSubmitError(
          getAccountAccessMessage(reason, false)
        );
      ...
      if (!sessionReady) {
        throw new Error(
          "Firebase authentication session could not be restored."
        );
      }

      console.log("BEFORE EMAIL/PASSWORD NAVIGATION:", {
        currentUser: auth.currentUser,
        uid: auth.currentUser?.uid,
        role,
      });
      ...
      if (
        ...
        setSubmitError(
          "Your login was verified, but the session could not be restored. Please try logging in again."
        );
      } else {
        setSubmitError(
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.log("GOOGLE PROFILE CHECK:", {
        success,
        reason,
        role,
        currentUser: auth.currentUser,
      ...
      if (!success) {
        ...
        setSubmitError(
          getAccountAccessMessage(reason, true)
        );
      ...
      console.log("BEFORE GOOGLE NAVIGATION:", {
        currentUser: auth.currentUser,
        uid: auth.currentUser?.uid,
        role,
      });
      ...
      if (
        ...
        setSubmitError(
          "Your Google account was verified, but the session could not be restored. Please try logging in again."
        );
      } else {
        setSubmitError(
          getFriendlyGoogleAuthError(error)
        );
      }
    } finally {
      ...
      <div className="col-span-1 md:col-span-7 lg:col-span-7 flex flex-col justify-center items-center px-6 py-12 sm:px-16 md:px-16 lg:px-24 bg-white h-full overflow-hidden">
        ...
        >
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-3 border border-gray-200 rounded-full py-3 font-semibold text-sm transition-all duration-200 ${isGoogleLoading
