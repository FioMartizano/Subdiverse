      ...
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center relative z-10">
        ...
        <div className="lg:col-span-6 space-y-6 order-2 lg:order-1">
          ...
          <motion.div 
            ...
            className="flex items-center gap-3 text-sm font-semibold tracking-widest uppercase text-amber-700"
          >
            <span>06</span>
            <span className="h-px w-12 bg-amber-600/50 inline-block" />
            <span>Faith & Fellowship</span>
          </motion.div>
          ...
          >
            The Parish <br />
            <span className="text-amber-600">Church Office.</span>
          </motion.h2>
          ...
          >
            Coordinates religious programs, community service ministries, and local parish activities. 
            Providing an open center for spiritual support and community gatherings, we seek to cultivate 
            deeper connections and neighborly bonds throughout Windward Hills.
          </motion.p>
        ...
        <div className="lg:col-span-6 relative flex justify-center lg:justify-end order-1 lg:order-2 h-[450px] md:h-[600px] w-full">
          ...
          {/* Amber Tracking Accent Trim Frame */}
          <motion.div 
            initial={{ opacity: 0, scale: 0.95, x: 20 }}
            whileInView={{ opacity: 1, scale: 1, x: 0 }}
            viewport={{ once: true, amount: 0.2 }}
            transition={{ ...smoothCurve, delay: 0.15 }}
            className="absolute inset-0 right-0 top-0 w-full h-full border-l-2 border-b-2 border-amber-600/40 rounded-bl-[10rem] md:rounded-bl-[16rem] pointer-events-none translate-x-[-12px] translate-y-[12px]"
          />
          ...
          >
            <img 
              src={churchGuest} 
              alt="Windward Hills Parish Church Sanctuary" 
          ...
          <div className="absolute bottom-8 left-8 w-6 h-12 border-2 border-slate-300 rounded-full hidden md:flex items-center justify-center backdrop-blur-sm bg-white/10">
            <div className="w-1.5 h-3 bg-amber-600 rounded-full animate-bounce" />
          </div>
